...
/* ────────────────────────────────────────────────────────────
 * Logging helpers  +  ANSI colour definitions
 * ──────────────────────────────────────────────────────────── */
...
static const char *tag_colour(const char *msg) {
    if (strstr(msg, "[RECEPTIONIST]")) { return CLR_CYN; }
    if (strstr(msg, "[SCHEDULER]"))    { return CLR_BLU; }
    if (strstr(msg, "[NURSE"))         { return CLR_MAG; }
    if (strstr(msg, "[SEMAPHORE]"))    { return CLR_YLW; }
    if (strstr(msg, "[STRATEGY]"))     { return CLR_GRN; }
    if (strstr(msg, "[PAGING]"))       { return CLR_DIM; }
    if (strstr(msg, "[FRAG]"))         { return CLR_DIM; }
    if (strstr(msg, "[PAGE TABLE]"))   { return CLR_DIM; }
    if (strstr(msg, "ALLOC_COMPARE"))  { return CLR_DIM; }
    if (strstr(msg, "WARD MAP"))       { return CLR_DIM; }
    if (strstr(msg, "==="))            { return CLR_GRN; }
    return CLR_RESET;
}
...
static void print_ward_map(BedMap *bm, const char *prefix) {
    ...
    for (int i = 0; i < bm->partition_count; i++) {
        ...
        len += snprintf(buf+len, sizeof(buf)-(size_t)len,
                        "[P%d:%s:%s(sz:%d)] ", p->partition_id, p->bed_type, status, p->size);
    }
...
static void append_mmap_record(const PatientRecord *rec, const char *action) {
    ...
    if (ftruncate(fd, size + entry_len) < 0) { close(fd); return; }
...
static void page_table_allocate(BedMap *bm, int start_unit, int size, int patient_id) {
    ...
    snprintf(msg, sizeof(msg),
             "[PAGING] Patient %d: %d units → %d pages (page_size=%d). Internal Fragmentation: %d wasted unit(s)",
             patient_id, size, pages_needed, PAGE_SIZE, wasted);
    write_log(msg);
}
...
static void report_fragmentation(BedMap *bm, const char *ward_type) {
    ...
    char msg[256];
    snprintf(msg, sizeof(msg),
             "[FRAG] Ward=%-10s FreeUnits=%d LargestFreeBlock=%d ExtFragmentation=%.2f%%",
             ward_type, free_units, largest_run, frag_pct);
    write_log(msg);
...
static int find_partition(BedMap *bm, const char *ward_type, int units, AllocStrategy strategy) {
    ...
    for (int i = 0; i < bm->partition_count; i++) {
        if (bm->partitions[i].is_free &&
            bm->partitions[i].size >= units &&
            strcmp(bm->partitions[i].bed_type, ward_type) == 0) {
...
static int allocate_bed(BedMap *bm, PatientRecord *rec) {
    ...
    log_admission("ALLOC_COMPARE", -1,
                  "Ward=%s FirstFitIdx=%d BestFitIdx=%d WorstFitIdx=%d ChosenIdx=%d Strategy=%d",
                  target, ff, bf, wf, p_idx, (int)g_alloc_strategy);

    if (p_idx == -1) return -1;

    /* Split if partition is larger than needed */
    if (bm->partitions[p_idx].size > rec->care_units) {
...
static void free_bed(BedMap *bm, int pat_id) {
    ...
    page_table_free(bm, bm->partitions[target_idx].start_unit,
                    bm->partitions[target_idx].size);

    bm->partitions[target_idx].is_free = 1;
    bm->partitions[target_idx].patient_id = -1;

    /* Coalesce right */
    if (target_idx < bm->partition_count - 1 &&
        bm->partitions[target_idx+1].is_free &&
        strcmp(bm->partitions[target_idx+1].bed_type, ward_type) == 0) {
...
static void *nurse_thread(void *arg) {
    ...
    while (g_running) {
        ...
        while (g_running && fgets(line, sizeof(line), fp)) {
            ...
            /* Determine which ward this patient belongs to */
            pthread_mutex_lock(&g_bedmap->lock);
            const char *found_ward = NULL;
            for (int i = 0; i < g_bedmap->partition_count; i++) {
                if (g_bedmap->partitions[i].patient_id == pat_id) {
                    found_ward = g_bedmap->partitions[i].bed_type;
                    break;
                }
            }

            /* Only process if this discharge belongs to our ward */
            if (!found_ward || strcmp(found_ward, na->ward_name) != 0) {
                pthread_mutex_unlock(&g_bedmap->lock);
                continue;
            }

            snprintf(logbuf, sizeof(logbuf),
                     "[NURSE-%s] Processing discharge for patient %d",
                     na->ward_name, pat_id);
            write_log(logbuf);
            ...
            /* Release the ward semaphore */
            if (strcmp(na->ward_name, "ICU") == 0)
                sem_post(&g_bedmap->icu_sem);
            else if (strcmp(na->ward_name, "Isolation") == 0)
                sem_post(&g_bedmap->iso_sem);
            else
                sem_post(&g_bedmap->gen_sem);
...
int main(int argc, char *argv[]) {
    ...
    printf("%s║%s     CL2006 OS Lab — Ahsan & Irzam                %s║%s\n", CLR_CYN, CLR_RESET, CLR_CYN, CLR_RESET);
    printf("%s╠══════════════════════════════════════════════════╣%s\n", CLR_CYN, CLR_RESET);
